...
int QLearning::ARGMAX(int s,int Max_actions)
{
    int amax = 0;
...
float QLearning::MAXQ(int s, int Max_actions)
{
    float max = 0;
    for(int a=0; a < Max_actions; a++)
...
void QLearning::Learning(int maxIterations, int maxValue, int n_states)
{
    ...
    {
        a0 = a; s0 = s;
        //Получить текущие состояние с сенсора
        s = getState(s0, a);
        freqTable[make_tuple(s0,a)]++;
        // Получить вознаграждение за предыдущее действие
        if( bBadAction )
            r = 0;
        else
        {
        ...
        if(s0 == s) // если был переход в невозможное состояние
            bBadAction = true;
        else
            bBadAction = false;
        ...
        a = ARGMAX(s,maxIterations); //Decision (Выбор действия):
