...
void Camera::key_pressed(int k)
{
    ...
    if(k == 4)
    {
        ...
        if(!flags.bdisable_timer2)
            sprintf(gstr,"executing script with update");
        else
            sprintf(gstr,"executing script no update ");
        //cosd.SetTopText(gstr, 50, num);
#endif
    }
    ...
    if(k == 3)
    {
        ...
        sprintf(gstr,"update = %d", !flags.bdisable_timer2);
        //cosd.SetTopText(gstr, 50, num);
    ...
    if(iinput->keystates['v']) //view
    {
        ...
        if( gselection.Count() > 0)
        {
            ...
            sprintf(gstr, "object %s selected", obj->name.c_str());
        }
        else
        {
            //default obj - veh01
...
void Camera::Slot1()
{
    ...
    char str[200];
    sprintf(str, "%dx%d(%f)", RES_X, RES_Y, aspect);
    //prn(str);
...
void Camera::DoScan()
{
    ...
    char str[30];
    sprintf(str,"cp=(%d %d %d %d)", GET_R(cp), GET_G(cp), GET_B(cp),GET_A(cp));
    pscr.upds("cp",str);
#endif
