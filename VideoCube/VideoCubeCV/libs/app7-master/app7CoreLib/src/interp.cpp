...
/*
     ...
     INPUTS:
          ...
          nf		array containing the coefficients of 
			...
			polynomial (obtained from ФdivdffХ)
          t		value of independent variable at which
			the interpolating polynomial is to be
			evaluated
...
double Time_domainRRR[1000000];
void Spline::SetSignal(double* f, double time, int Res)
{
		double t=0;
		double dt = time/Res;
		for(int i=0; i < Res; i++) {	Time_domainRRR[i] = t;		t+=dt;	 }
		
		//генерируем массивы коэффициентов сплайна
		SetN(Res);
		SetXY(Time_domainRRR, f);
		Calc();
 }
