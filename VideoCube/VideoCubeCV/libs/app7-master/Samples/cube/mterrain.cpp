...
void MTerrain::Create(int w_segs, int h_segs, int l_segs, float sizex, float sizey, float sizez)
{
    ...
    if(terrain_mesh==0)
    {
        ...
        //terrain_mesh->ei.texture_name = "brick0";
        //terrain_mesh->ei.texture_id = mater.GetTexureID("brick0");
    }
    ...
    for(int k=0; k < l_segs; k++)
        for(int j=0; j < h_segs; j++)
            for(int i=0; i < w_segs; i++)
            {
                ...
                terrain_mesh->vertices[vcount].texCoord = QVector2D(0, 0);
                if( j == h_segs-1 )
                {
                    QPair<int,int> idx;
                    idx.first = i;
...
void MTerrain::SetBumpImage(QImage& I)
{
    ...
    double dw = I.width() / mprops.w_segs;
    double dl = I.height() / mprops.l_segs;
    for(int k=0; k < mprops.l_segs; k++)
       for(int i=0; i < mprops.w_segs; i++)
       {
...
void MTerrain::AssignTextureCoords()
{
    ...
    for(int k=0; k < mprops.l_segs; k+=2)
       for(int i=0; i < mprops.w_segs; i+=2)
       {
           ...
           *A->getTexCoord0() = facet[0].x(); *A->getTexCoord1() = facet[0].y();
           *B->getTexCoord0() = facet[1].x(); *B->getTexCoord1() = facet[1].y();
           *C->getTexCoord0() = facet[2].x(); *C->getTexCoord1() = facet[2].y();
           *D->getTexCoord0() = facet[3].x(); *D->getTexCoord1() = facet[3].y();
