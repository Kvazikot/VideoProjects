...
void MazeGen::DrawCells(QImage& image)
{
    ...
    for(int j=0; j < cellsY; j++)
    {
       if( cells.find(std::make_tuple(i,j))!= cells.end() )
       {
           ...
           {
               QRect wall_rc( C.x() - cell_size/2 - wall_width/2, C.y() - cell_size/2 , wall_width, cell_size + wall_width );
           ...
           {
               QRect wall_rc( C.x() - cell_size/2 , C.y() - cell_size/2 - wall_width/2, cell_size + wall_width, wall_width );
               //painter.drawRect(wall_rc);
               painter.fillRect(wall_rc, Qt::blue);
           }
