...
float MazeGen::getCellPixelWidth()
{
    Cell* cell = getCell(0,0);
    Cell* cell2 = getCell(0,1);
    return cell->zworld - cell2->zworld;
}
...
Cell* MazeGen::getCell(int i, int j)
{
    if( cells.find(make_tuple(i,j))!=cells.end())
        return cells[make_tuple(i,j)];
    else
        return &null_cell;
}
...
void MazeGen::DrawCells(QImage& image)
{
    ...
    for(int j=0; j < cellsY; j++)
    {
       if( cells.find(std::make_tuple(i,j))!= cells.end() )
       {
           ...
           {
               QRect wall_rc( C.x() - cell_size/2  , C.y() - cell_size/2 , cell_size + wall_width, wall_width );
               //painter.drawRect(wall_rc);
               painter.fillRect(wall_rc, Qt::blue);
           }
