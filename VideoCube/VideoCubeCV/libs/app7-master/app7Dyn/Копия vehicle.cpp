...
void CVehicle::InputControl()
{
    ...
    if( iinput.keystates[186] ) // вправо
    {
        //mu_printf("right");
        //kurs.x+=5;
        if(wang>-max_wang)
...
void CVehicle::FitWheesInPlane(Pnt& w0p, Pnt& w1p, Pnt& w2p, Pnt& w3p )
{
	...
	float  dy = 0.001;   // шаг по высоте
	float  prec = dy*0.1; // точность подгонки (влияет на количество шагов при поиске
	Pnt R0,R1,R2,R3;
