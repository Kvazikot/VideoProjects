...
class MVP(transforms.MVPTransform):
    def __init__(self, shader_program_id, aspect):
        super().__init__()
        self.name = 'MVP'
        self.shader_program_id = shader_program_id
        self.projection.aspect = aspect
        self.projection.far = 100.
        self.view.eye = np.array((4,3,3))

    @property
    def id(self):
        return gl.glGetUniformLocation(self.shader_program_id, self.name)
...
def create_main_window(width, height):
    ...
    try:
        ...
        title = 'Tutorial 4: Model-View-Projection'
...
def main_loop(window, mvp):
    while (
        ...
        # Set the view matrix
        gl.glUniformMatrix4fv(mvp.id, 1, False, mvp.matrix())
    ...
    with create_main_window(width, height) as window:
        with create_vertex_buffer():
            with load_shaders() as prog_id:
                mvp = MVP(prog_id, width / height)
                main_loop(window, mvp)
