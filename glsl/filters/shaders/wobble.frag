...
void main (void)
{
    ...
    rad = (gl_TexCoord[0].s + gl_TexCoord[0].t) - 1.0 + r *4.0;
    ...
    rad = (gl_TexCoord[0].s - gl_TexCoord[0].t + r) * 4.0;
    ...
    vec3 lightColor = vec3(texture2D(sampler2d, perturb + gl_TexCoord[0].st)) * lightIntensity;

    vec3 ct = clamp(lightColor, 0.0, 1.0);

    gl_FragColor = vec4 (ct, 1.0);
   //gl_FragColor = texture2D(sampler2d, gl_TexCoord[0].st);
}
